function [nrmseV,preM, residuals] = localfitnrmse(xV,tau,m,Tmax,nnei,q,tittxt)
    ...
    for i=1:nvec
        ...
        if q==0 || nnei==1
            ...
            [Ux, Sx, Vx] = svd(zM, 0);
            tmpM = Vx(:,1:q) * pinv(Sx(1:q,1:q)) * Ux(:,1:q)';
            lsbV = tmpM * (yV - my);
    ...
    % Add the target point index before the 
    % iterative predictions nrmseV = NaN*ones(Tmax,1);
    preM = [(1:nvec)+(m-1)*tau; preM']'; 
    ...
    % compute residuals for 1 step ahead prediction 
    residuals = xV(preM(:, 1)+1) - preM(:, 1+1);
