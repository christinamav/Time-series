function [nrmseV, preM, residuals] = localpredictnrmse(xV, nlast, tau, m, Tmax, nnei, q, tittxt)
    ...
    % 
    v = reshape(((n1+1):n), [], 1);
    preM = [v, preM]; % Add the target point index before the 
                                  % iterative predictions
    nrmseV = NaN*ones(Tmax,1);

    for T=1:Tmax
        nrmseV(T) = nrmse(xV(preM(:,1)),preM(:,T+1));
    end
    

    residuals = xV(preM(:, 1)) - preM(:, 1 + 1);
